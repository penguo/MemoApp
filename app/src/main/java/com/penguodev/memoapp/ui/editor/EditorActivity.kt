...
class EditorActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val id = intent.getLongExtra(KEY_ID, INVALID_ID) // KEY_ID에 값이 있다면, 해당 값을 가지고 없다면 INVALID_ID 값을 가진다.
                .takeIf { it != INVALID_ID } // 안의 조건문이 true일 때만 값을 유지한다. 조건문이 false일 경우 null을 반환.

        viewModel = ViewModelProvider(this, EditorViewModel.Factory(this.application, id)).get(EditorViewModel::class.java)
